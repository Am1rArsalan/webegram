...
export class DirectsAgentImpl extends BaseAgentImpl implements DirectsAgent {
	...
	addToDirects(id: string) {
		return this.createFetchHttpRequest<DirectApiType>('PUT', `/direct/invite?id=${id}`, 'data');
	}

	fetchDirects() {
		return this.createFetchHttpRequest<DirectsApiType>('GET', '/direct/directs', 'data');
	}
